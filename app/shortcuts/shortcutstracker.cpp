...
void ShortcutsTracker::initGlobalShortcutsWatcher()
{
    ...
    const QString globalShortcutsFilePath = Latte::configPath() + QLatin1String("/") + GLOBALSHORTCUTSCONFIG;
...
void ShortcutsTracker::parseGlobalShortcuts()
{
    ...
    if (latteGroup.exists()) {
        ...
        qCDebug(latteLayout) << "badges based on position updated to :: " << m_badgesForActivate;
        qCDebug(latteLayout) << "badges for applet shortcuts updated to :: " << m_appletShortcuts;
